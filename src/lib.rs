...
mod direct;
pub use direct::{Direct, DirectMut, Director};
/// Direct and Visit implementations for the syn AST
#[cfg(feature = "syn")]
pub mod syn;
mod visit;
pub use visit::{Visit, VisitMut, Visitor};
...
where
    ...
    N: ?Sized,
{
    V::visit(Visitor::new(DirectorVisitor { direct, visit }), node);
}

/// Perform a visit using a [DirectMut]-[VisitMut] pair, and a given input.
pub fn visit_mut<'dv, D, V, N>(direct: &'dv mut D, visit: &'dv mut V, node: &mut N)
where
    D: DirectMut<V, N> + ?Sized,
    V: VisitMut<N> + ?Sized,
    N: ?Sized,
{
    V::visit_mut(Visitor::new(DirectorVisitor { direct, visit }), node);
}
...
mod tests {
    ...
    impl crate::Visit<A> for MyVisit {
        fn visit<'dv, D: ?Sized>(mut visitor: Visitor<'dv, D, Self>, node: &A)
        where
            D: Direct<Self, A>,
        {
            visitor.0 += 1;
    ...
    impl crate::Visit<B> for MyVisit {
        fn visit<'dv, D: ?Sized>(mut visitor: Visitor<'dv, D, Self>, node: &B)
        where
            D: Direct<Self, B>,
        {
            visitor.0 += 2;
    ...
    impl crate::Visit<C> for MyVisit {
        fn visit<'dv, D: ?Sized>(mut visitor: Visitor<'dv, D, Self>, node: &C)
        where
            D: Direct<Self, C>,
        {
            visitor.0 += 3;
